...
async function main(){
  ...
  const users=[
    {name:'Daniel Rosinski',email:'daniel@empresa.com',role:UserRole.EMPLOYEE,departmentId:departments['Administrativo'].id},
    {name:'Marcos Almeida',email:'marcos@empresa.com',role:UserRole.AGENT,departmentId:departments['Tecnologia da Informação'].id},
    {name:'Fernanda Souza',email:'fernanda@empresa.com',role:UserRole.AGENT,departmentId:departments['Financeiro'].id},
    {name:'Carlos Mendes',email:'carlos@empresa.com',role:UserRole.ADMIN,departmentId:null}
  ]
  const saved:any={}
  ...
  if(await prisma.ticket.count()===0){
    const samples=[
      ['CH-0001','Falha ao acessar sistema interno','Desde esta manhã não consigo acessar o sistema interno. A página retorna erro depois do login.','Sistema','Tecnologia da Informação',TicketPriority.HIGH,TicketStatus.IN_PROGRESS,saved['marcos@empresa.com'].id],
      ...
      ['CH-0004','Erro no lançamento de reembolso','O sistema informa valor inválido ao lançar o reembolso.','Reembolso','Financeiro',TicketPriority.HIGH,TicketStatus.RESOLVED,saved['fernanda@empresa.com'].id],
    ...
    for(const [code,title,description,categoryName,departmentName,priority,status,assignedToId] of samples){
      ...
      const ticket=await prisma.ticket.create({data:{code,title,description,categoryId:category.id,departmentId:departments[departmentName].id,priority,status,assignedToId,requesterId:saved['daniel@empresa.com'].id,resolvedAt:status===TicketStatus.RESOLVED?new Date():null}})
      await prisma.ticketHistory.create({data:{ticketId:ticket.id,userId:saved['daniel@empresa.com'].id,action:'TICKET_CREATED'}})
      if(code==='CH-0001') await prisma.ticketComment.createMany({data:[{ticketId:ticket.id,authorId:saved['daniel@empresa.com'].id,message:description},{ticketId:ticket.id,authorId:saved['marcos@empresa.com'].id,message:'Verifiquei seu usuário. O acesso estava bloqueado após algumas tentativas incorretas. Fiz a liberação. Tente novamente.'}]})
    }
  }
}
